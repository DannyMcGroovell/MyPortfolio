...
class FCurvesEvaluator(object):
    """Создает кривые линии для векторной анимации"""
...
class BakingOperator(object):
    ...
    def _bake_action(self, context, *source_bones):
        ...
        # Blender 2.81 : странение ошибки в операторе bake
        # удаление из выделения объектов, которые не являются текущими
...
class ANIM_OT_carWheelsRotationBake(bpy.types.Operator, BakingOperator):
    ...
    bl_description = 'Автоматически создает анимацию вращения колес на основе анимации кости "Root".'
...
class ANIM_OT_carSteeringBake(bpy.types.Operator, BakingOperator):
    ...
    bl_description = 'Автоматически создает анимацию поворота колес на основе анимации кости "Root".'
    bl_options = {'REGISTER', 'UNDO'}
...
class ANIM_OT_carClearSteeringWheelsRotation(bpy.types.Operator):
    ...
    bl_description = "Удалить созданную анимацию поворота и вращения колес"
    bl_options = {'REGISTER', 'UNDO'}

    clear_steering: bpy.props.BoolProperty(name="Steering", description="Удалить созданную анимацию поворота колес", default=True)
    clear_wheels: bpy.props.BoolProperty(name="Wheels", description="Удалить созданную анимацию вращения колес", default=True)

    def draw(self, context):
        self.layout.use_property_decorate = False
        self.layout.label(text='Удалить созданные ключевые кадры для')
