...
class ArmatureGenerator(object):
    ...
    def generate(self, scene, adjust_origin):
        define_custom_property(self.ob,
                               ...
                               value=False,
                               description="Включить вращение колес при движении кости 'Root' вдоль оси Y")
        define_custom_property(self.ob,
                               name='suspension_factor',
                               value=.5,
                               description="Влияние амортизаторов на наклон кузова")
        define_custom_property(self.ob,
                               name='suspension_rolling_factor',
                               value=.5,
                               description="Влияние амортизаторов на крен кузова")
    ...
    def generate_animation_wheel_bones(self, name_suffix, wheel_bounding_box, parent_bone):
        ...
        define_custom_property(self.ob,
                               ...
                               value=.0,
                               description="Свойство анимации вращения колес")
    ...
    def generate_constraints_on_rig(self):
        ...
        if steering is not None:
            ...
            define_custom_property(self.ob,
                                   ...
                                   value=.0,
                                   description="Свойство анимации для поворота колес")
...
class OBJECT_OT_armatureCarDeformationRig(bpy.types.Operator):
    ...
    bl_label = "Добавить скелет автомобиля"
    bl_description = "Создается базовый скелет для автомобиля"
    bl_options = {'REGISTER', 'UNDO'}

    body_pos_delta: bpy.props.FloatVectorProperty(name='Delta Location',
                                                  description='Дополнительный перевод к расположению кузова',
                                                  size=3,
                                                  default=(0, 0, 0),
                                                  subtype='TRANSLATION')

    nb_front_wheels_pairs: bpy.props.IntProperty(name='Pairs',
                                                 description='Количество пар передних колес',
                                                 default=1,
                                                 min=0)

    front_wheel_pos_delta: bpy.props.FloatVectorProperty(name='Delta Location',
                                                         description='Дополнительный перевод к расположению передних колес',
                                                         size=3,
                                                         default=(0, 0, 0),
                                                         subtype='TRANSLATION')

    nb_back_wheels_pairs: bpy.props.IntProperty(name='Pairs',
                                                description='Количество пар задних колес',
                                                default=1,
                                                min=0)

    back_wheel_pos_delta: bpy.props.FloatVectorProperty(name='Delta Location',
                                                        description='Дополнительный перевод к расположению задних колес',
                                                        size=3,
                                                        default=(0, 0, 0),
                                                        subtype='TRANSLATION')

    nb_front_wheel_brakes_pairs: bpy.props.IntProperty(name='Front Pairs',
                                                       description='Количество пар передних тормозных дисков',
                                                       default=0,
                                                       min=0)

    front_wheel_brakes_pos_delta: bpy.props.FloatProperty(name='Front Delta Location',
                                                          description='Дополнительный перевод к расположению передних тормозных дисков',
                                                          default=0)

    nb_back_wheel_brakes_pairs: bpy.props.IntProperty(name='Back Pairs',
                                                      description='Количество пар задних тормозных дисков',
                                                      default=0,
                                                      min=0)

    back_wheel_brakes_pos_delta: bpy.props.FloatProperty(name='Back Delta Location',
                                                         description='Дополнительный перевод к расположению задних тормозных дисков',
                                                         default=0)
    ...
    def invoke(self, context, event):
        ...
        # если целевой объект для кузова не найден, то предполагаем, что целевого объекта
        # для передних и задних колес тоже нет
    ...
    def execute(self, context):
        """Создает мета-скелет с базовыми костями"""
        ...
        try:
            ...
            self.report({'ERROR'}, "Невозможно изменить новый скелет! Пожалуйста, убедитесь, что активная коллекция видна и доступна для редактирования.")
            return {'CANCELLED'}
...
class POSE_OT_carAnimationRigGenerate(bpy.types.Operator):
    ...
    bl_description = "Создает полностью готовый скелет для анимирования автомобиля."
    bl_options = {'REGISTER', 'UNDO'}

    adjust_origin: bpy.props.BoolProperty(name='Move origin',
                                          description='Установить исходную точку скелета там же, где находится исходная точка кости "Root"',
                                          default=True)
    ...
    def execute(self, context):
        if context.object.data['Car Rig']:
            self.report({'INFO'}, 'Скелет уже создан')
            return {"CANCELLED"}

        if 'DEF-Body' not in context.object.data.bones:
            self.report({'ERROR'}, 'Нет кости с именем DEF-Body. Этот скелет недопустим!')
            return {"CANCELLED"}
...
class POSE_OT_carAnimationAddBrakeWheelBones(bpy.types.Operator):
    ...
    bl_label = "Добавить недостающие кости для тормозных дисков"
    bl_description = "Создает недостающие кости тормозных дисков для каждого из выбранных виджетов колеса."
    bl_options = {'UNDO'}
